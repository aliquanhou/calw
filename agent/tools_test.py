"""tools_test — 测试驱动循环：发现、运行、分析、修复闭环。"""

from __future__ import annotations

import json
import os
import re
import subprocess
import sys
import time


def discover_tests(path: str | None = None) -> dict:
    """自动发现项目中的测试文件和测试用例。

    Returns:
        {"framework": "pytest|unittest", "files": [...], "total": N}
    """
    root = path or os.getcwd()
    test_files = []
    framework = "unknown"

    for dirpath, dirnames, filenames in os.walk(root):
        # 跳过忽略目录
        dirnames[:] = [d for d in dirnames if d not in (
            ".git", "__pycache__", "node_modules", ".venv", "venv",
            ".env", ".tox", "build", "dist", ".idea", ".vscode",
        )]
        for f in filenames:
            if f.startswith("test_") and f.endswith(".py"):
                rel = os.path.relpath(os.path.join(dirpath, f), root)
                test_files.append(rel)
            elif f.endswith("_test.py"):
                rel = os.path.relpath(os.path.join(dirpath, f), root)
                test_files.append(rel)

    # 检测框架
    conftest = os.path.join(root, "conftest.py")
    if os.path.exists(conftest) or any("conftest" in f for f in test_files):
        framework = "pytest"
    elif any(os.path.basename(f).startswith("test_") or f.endswith("_test.py") for f in test_files):
        framework = "pytest"
    else:
        # 检查是否有 unittest 风格的导入
        for tf in test_files[:5]:
            fp = os.path.join(root, tf)
            try:
                content = open(fp, "r", encoding="utf-8", errors="replace").read()
                if "unittest" in content and "TestCase" in content:
                    framework = "unittest"
                    break
            except Exception:
                pass

    return {
        "framework": framework,
        "files": sorted(test_files),
        "total": len(test_files),
    }


def run_tests(path: str | None = None, test_name: str = "", timeout: int = 300) -> dict:
    """运行测试并解析结果。

    Args:
        path: 测试文件或目录路径，默认当前目录
        test_name: 特定测试名（如 "test_foo" 或 "test_file.py::test_func"）
        timeout: 超时秒数

    Returns:
        结构化结果：摘要 + 失败详情 + 逐用例结果
    """
    root = path or os.getcwd()
    result = {
        "passed": 0,
        "failed": 0,
        "errors": 0,
        "total": 0,
        "duration": 0.0,
        "failures": [],
        "framework": "pytest",
        "raw_output": "",
    }

    start = time.time()

    # 构造 pytest 命令
    cmd = [sys.executable, "-m", "pytest", "-v", "--tb=short", "--no-header"]
    if test_name:
        # 如果 test_name 是函数名，转为 pytest -k 参数
        if "::" in test_name:
            cmd.append(test_name)
        elif test_name.endswith(".py"):
            cmd.append(os.path.join(root, test_name))
        else:
            cmd.extend(["-k", test_name])
    else:
        cmd.append(root)
    cmd.extend(["-p", "no:cacheprovider"])

    try:
        proc = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
            timeout=timeout,
        )
        raw = proc.stdout + "\n" + proc.stderr
    except subprocess.TimeoutExpired:
        return {
            "passed": 0, "failed": 0, "errors": 1, "total": 0,
            "duration": time.time() - start, "failures": [],
            "framework": "pytest", "raw_output": "TIMEOUT",
        }
    except FileNotFoundError:
        return {
            "passed": 0, "failed": 0, "errors": 1, "total": 0,
            "duration": time.time() - start, "failures": [],
            "framework": "pytest", "raw_output": "pytest not installed",
        }

    result["duration"] = round(time.time() - start, 2)
    result["raw_output"] = raw[-5000:]  # 保留最后 5000 字符

    # 解析 pytest 输出
    # 行模式: test_file.py::test_func PASSED/FAILED/ERROR
    test_case_pattern = re.compile(
        r"^(.+?)::(.+?) (PASSED|FAILED|ERROR)", re.MULTILINE
    )
    for m in test_case_pattern.finditer(raw):
        filepath = m.group(1)
        test_func = m.group(2)
        status = m.group(3)
        result["total"] += 1
        if status == "PASSED":
            result["passed"] += 1
        elif status == "FAILED":
            result["failed"] += 1
            # 提取失败的详细信息
            failure_detail = _extract_failure(raw, test_func)
            result["failures"].append({
                "file": filepath,
                "test": test_func,
                "detail": failure_detail,
            })
        elif status == "ERROR":
            result["errors"] += 1
            result["failures"].append({
                "file": filepath,
                "test": test_func,
                "detail": _extract_failure(raw, test_func),
            })

    # 如果正则没匹配到，尝试解析概要行
    if result["total"] == 0:
        summary_match = re.search(
            r"(\d+) passed.*?(\d+) failed.*?(\d+) error",
            raw, re.DOTALL
        )
        if summary_match:
            result["passed"] = int(summary_match.group(1)) if summary_match.group(1) else 0
            result["failed"] = int(summary_match.group(2)) if summary_match.group(2) else 0
            result["errors"] = int(summary_match.group(3)) if summary_match.group(3) else 0
            result["total"] = result["passed"] + result["failed"] + result["errors"]
        else:
            # 可能全部通过或全部失败
            passed_match = re.search(r"(\d+) passed", raw)
            failed_match = re.search(r"(\d+) failed", raw)
            if passed_match:
                result["passed"] = int(passed_match.group(1))
            if failed_match:
                result["failed"] = int(failed_match.group(1))
            result["total"] = result["passed"] + result["failed"] + result["errors"]

    return result


def _extract_failure(raw_output: str, test_name: str) -> str:
    """从 pytest 输出中提取指定测试的失败详情。"""
    # 找到测试名称后的 FAILURES 段
    lines = raw_output.split("\n")
    capture = False
    detail_lines = []
    # 搜索 "FAILED test_file::test_name" 或 "FAILURES" 段
    target_pattern = re.compile(rf"FAILED.*{re.escape(test_name)}")
    failures_header = False

    for i, line in enumerate(lines):
        if "= FAILURES =" in line:
            failures_header = True
            continue
        if failures_header:
            if line.startswith("_" * 10):  # ____ 分隔线
                if capture:
                    break
                capture = True
                continue
            if capture:
                detail_lines.append(line.rstrip())
                if len(detail_lines) > 50:  # 限制长度
                    detail_lines.append("... (截断)")
                    break
        # 直接匹配 FAILED 行
        if target_pattern.search(line):
            # 取后面的 10 行作为错误信息
            for j in range(i + 1, min(i + 15, len(lines))):
                l = lines[j].rstrip()
                if not l or l.startswith("="):
                    break
                detail_lines.append(l)

    result = "\n".join(detail_lines).strip()
    return result[:2000] if result else "(无详细错误信息)"


def _handle_test(action: str = "run", path: str = "", test_name: str = "",
                 timeout: int = 300) -> str:
    """test 工具的主入口。"""
    if action == "discover":
        info = discover_tests(path or None)
        if info["total"] == 0:
            return "未发现测试文件。"
        lines = [
            f"框架: {info['framework']}",
            f"测试文件数: {info['total']}",
        ]
        for f in info["files"][:30]:
            lines.append(f"  🧪 `{f}`")
        if len(info["files"]) > 30:
            lines.append(f"  ... 还有 {len(info['files']) - 30} 个")
        return "\n".join(lines)

    elif action == "run":
        r = run_tests(path or None, test_name, timeout)

        lines = [
            f"📊 测试结果 ({r['framework']})",
            f"   总计: {r['total']} | ✅ 通过: {r['passed']} | ❌ 失败: {r['failed']} | ⚠️ 错误: {r['errors']}",
            f"   耗时: {r['duration']}s",
        ]

        if r["failures"]:
            lines.append("")
            lines.append(f"--- 失败详情 ({len(r['failures'])} 个) ---")
            for i, f in enumerate(r["failures"][:5]):
                lines.append(f"")
                lines.append(f"#{i+1} {f['file']}::{f['test']}")
                detail = f["detail"]
                if detail:
                    for d_line in detail.split("\n")[:8]:
                        lines.append(f"  {d_line}")
                if len(r["failures"]) > 5 and i == 4:
                    lines.append(f"  ... 还有 {len(r['failures']) - 5} 个失败")

        return "\n".join(lines)

    return f"未知操作: {action}"
